...
tags:
  - rust
  - algo
ogImageUrl: "https://res.cloudinary.com/dmdgv6qrg/image/upload/v1700134021/hhai.dev%20blog/adding%20comments/og-image.png"
---
...
Our recursive iterative ranking function will receive a list of comments as
input and output a sorted list. Firstly, it will do a loop of:

- Find all child of the current comment - this takes $O(n)$ where $n$ is number
of children of the current comment. We do this by incrementing the index until
we get a node whose level is equal to or higher than the current node. Call this
node `next` and forget about it for now.
...
where $m$ is number of top level comments.
