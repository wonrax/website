...
I also have an API written in Rust (yes, I'm on the hype train too) and a
PostgreSQL just for increase the view count for each post. I better make more
use of that. So I decided to implement my own comment platform. This means I
have to take care of many things (not supposed to be an exhausted list):
...
It may sound ridiculously complex but it's just me designing database schemas
after researching for a suitable model for storage and easy querying. Below is
schema in Prisma and a corresponding SQL migration script.
...
:::note
I use Prisma schema but it doesn't mean I'm using the client for API. Prisma
just has a very good DX for migration and dev stuff. I only use the client for
seed scripts. For the backend, I use [Rust
sqlx](https://github.com/launchbadge/sqlx) and write plane SQL like a normal
person.

`prisma db push` to push the new schema to the database, `db pull` to pull the
database schema and update the schema file. So making changes to the schema is
quite easy.

When satisfied with the new schema, I run `migrate dev` to generate migration
files. And on production, I spin a new docker container to run `migrate deploy`
to make changes to the production database.
:::
...
I implemented both solutions and did a benchmark. Below is a table showing the
time each implementation took in average for input of `n` comments.
...
Below is the code. I use Rust criterion for benchmarking. It's a bit sad that
`cargo bench` is still under-development and I had to copy every implementations
code for benchmarking (it's not possible to import because my Rust project is
binary, not lib).
...
I think both implementations can be further optimized. Like the
`iterative_recursive` could use slices instead of Vec (?). But I think it's
enough for my blog and stop.

:::note
At the time of writing this post, I only finished the API part, not the UI and
moderation and stuff (it still has a very long way ahead), so it's still not
possible to create comments.

You'll know when it's available 🙂
:::
