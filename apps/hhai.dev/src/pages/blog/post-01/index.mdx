...
is precisely why it has become the default language choice for document writing
among software engineers. Its design emphasizes readability in plain text,
allowing you to effortlessly read and edit raw text without relying on a parser
like [Pandoc](https://pandoc.org/) to convert it into HTML or a renderer such as a browser to view the
formatted HTML.
...
This post demonstrates my customized Markdown-to-blog post process, serving as
both documentation and a guide for future reference, saving me from revisiting
the code.
...
GFM is the default formatting in [Astro](https://astro.build/), the framework I
used for this site.
...
```go showLineNumbers title="example.go" {4-6, 1} /Address/
package main
