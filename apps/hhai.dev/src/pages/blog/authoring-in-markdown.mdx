...
I'm so used to writing Markdown on GitHub that I didn't even know I didn't
realize they were extensions, not part of standard Markdown. GFM is the default
formatting in [Astro](https://astro.build/), the framework powering site.
...
probably doesn't look good too. The ideal image resolution matches the width and
height of the rendered element in the browser. For example, if you set an image
to display on your site at 1600px wide, it looks the sharpest when the actual
image's width is exactly 1600px. The method to achieve this is called responsive
image and MDN has [a great
...
image hosting services so you can just put a query parameter in the end of the
...
actually have traffic. That's also why I moved off from Next.js, they have
`export` mode which generates static files, thus doesn't need a Next.js server
to run. But this mode doesn't support pre-generating images at build time.
There's [a discussion](https://github.com/vercel/next.js/discussions/19065) that
addresses this and despite being one of the most requested features since 2020,
they still haven't implemented it. Maybe because it's incredibly hard to change
the internal without breaking things. I've also heard some people claim it's due
to Vercel's image optimization service or something, which I don't really
understand – I'm not exactly a business guy.
...
transform them into custom resolutions. However, the API demands width and
...
additional line breakss)

<img src="https://share.hhai.dev/IMG_1203.png" width="1920" height="816" />
> With the <img> tag, you need to put the caption inside block quote
```
...
![img](https://share.hhai.dev/IMG_1203.png
'#width=1920;height=816;featuretype=sm') Remote image with `featuretype=sm`

![img](https://share.hhai.dev/IMG_1219.png
...
This could very well be my last post. Ironically, what's the purpose of all
these efforts if I don't keep writing?
