...
description: "In defense of dependencies and package managers."
...
up on [Hacker News](https://news.ycombinator.com/item?id=43935067) and
[elsewhere](https://www.reddit.com/r/rust/comments/1ki3kzj/rust_dependencies_scare_me/)
about Rust's dependency hell. The post presents nothing new but a familiar
challenge in modern software development regardless of the language and
ecosystem. While this is reasonable, I've noticed people often fear and
overreact to concepts they don’t fully grasp. This reaction risks being
counterproductive and even harmful at some point. The discussions did push me
to reconsider dependencies in a broader sense and how we can improve them
without sacrificing their convenience. I believe the answer lies in truly
understanding their role and embracing the complexity they introduce.

This post is a defense of dependencies and package managers.

## Dependencies

Foremost, it's important to agree on what constitutes dependencies.
Dependencies aren't just the libraries you pull into your project, but also the
tools used to build and run your software. This includes the compiler, build
system, dynamically linked libraries, and the operating system itself. In other
words, dependencies are everything your software relies on to function
correctly. Hardware is also a dependency, but this post stops at the software
boundary.

[LetsBeRealAboutDependencies](
https://wiki.alopex.li/LetsBeRealAboutDependencies) points out that softwares
that don't use a package manager still depend on a lot of dynamic libraries
that made available by the operating system or its package manager. These two
types of dependencies are not mutually exclusive. In rust, for example, you can
have both static and dynamic dependencies: the Rust standard library relies on
`libc`, while third-party libraries may depend on dynamic ones like `openssl`
or `libpq`. What's important is, whether statically or dynamically linked, they
remain dependencies.

When we choose a language to build software, we've already chosen to depend on
its abstractions and become tied to its ecosystem. When designing software for
a specific platform, we inherently depend on that platform unless
cross-platform abstractions exist to avoid fragmentation.
...
> and still have something that can be portable to other machines." -- [AT&T
> Archives: The UNIX Operating
> System (1982)](https://www.youtube.com/watch?v=tc4ROCJYbm0)

### The bads


> If I understand correctly how it works, when cargo goes to build
> dependencies, it automatically picks up bug fixes in transitive dependencies
> (semver)

> Software quality is at its historical worst. (JonBlow and Casey)
Well this rant is not new. It's because softwares are more accessible than
ever. While before people have to learn how to use command line and composing
unix utilities to perform their tasks, even programming in bash.
...
- https://www.youtube.com/watch?v=kZRE7HIO3vk makes a good argument about
rethinking how we interface with hardware in a radical way "in order to avoid
having to rely on abstraction layers on top of abstraction layers" but it
doesn't seem to be practical at least in the short run, and "implement the higher levels in software because it's more flexible and easier to update, and the cpu load isn't that much"
